...
PathFinding::PathFinding(const dyadic_array & map, GridPoint start, GridPoint destination)
{
	...
	_map[start._x][start._y].setFlag(START);
	_map[destination._x][destination._y].setFlag(DESTINATION);

	_starting_point = &_map[start._x][start._y];
	_terminal_point = &_map[destination._x][destination._y];
}
	...
	for (int i = 0; i < 8; ++i) {
		...
		if (isInMapRange(current_grid) && !isCorner(grid, _map[cx][cy]) && isAvailable(_map[cx][cy])) {
			...
			if (_map[cx][cy].getFlag() == OPEN) {
				...
				_map[cx][cy].setG(g);
				_map[cx][cy].setF(g + _map[cx][cy].getH());
				_open_list.push_back(&_map[cx][cy]);
			}
		}
	}
}
